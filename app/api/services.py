...
def create_request_with_parts(
    *,
    ...
    part_count = int(
        getattr(settings, "CRACKHASH_PART_COUNT", 4)
    )

    timeout_seconds = int(
        getattr(settings, "CRACKHASH_REQUEST_TIMEOUT_SECONDS", 300)
    )
    ...
    CrackPart.objects.bulk_create(parts)

    enqueue_request_parts(str(req.id))
